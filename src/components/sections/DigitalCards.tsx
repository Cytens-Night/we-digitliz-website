...
const CLIENTS = [
  ...
  { 
    ...
    logoStyle: "font-display tracking-tight text-2xl md:text-4xl font-black text-white drop-shadow-[0_0_15px_rgba(0,122,255,0.5)]"
  },
  { 
    id: "hesori", 
    name: "Hesori", 
    ...
    logoStyle: "font-sans tracking-[0.4em] uppercase text-xl md:text-2xl font-light text-white"
  },
  { 
    id: "marshalos", 
    name: "Marshalos", 
    ...
    logoStyle: "font-display tracking-widest uppercase text-2xl md:text-3xl font-bold italic text-white"
  }
];
    ...
    <section id="portfolio" className="relative bg-[#0a0a0a] text-white py-24 md:py-32 overflow-hidden border-t border-white/5">
      ...
      <div className="max-w-[1400px] mx-auto px-6 relative z-10">
        ...
        <div className="mb-20 flex flex-col items-center text-center gap-6">
           <div className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-white/5 text-white/90 text-xs font-bold border border-white/10 uppercase tracking-widest shadow-[0_0_20px_rgba(255,255,255,0.05)]">
              Client Testimonials
           </div>
           <h2 className="text-4xl md:text-6xl lg:text-7xl font-display font-bold text-white max-w-4xl tracking-tight leading-[1.1]">
             What our clients <br/> <span className="text-white/40">say about us.</span>
           </h2>
        </div>
        ...
        <div className="hidden md:grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
           {CLIENTS.map((client, index) => (
             <motion.div 
               ...
               className="group relative h-[400px] lg:h-[450px] rounded-[2rem] overflow-hidden bg-[#121212] border border-white/10 flex flex-col p-8 hover:border-white/30 transition-all duration-500 shadow-[0_20px_40px_rgba(0,0,0,0.5)] hover:shadow-[0_30px_60px_rgba(0,122,255,0.15)]"
             >
                ...
                {/* Quote Icon */}
                <div className="absolute top-8 right-8 text-white/5 group-hover:text-white/10 transition-colors duration-500 z-10">
                ...
                <div className="relative z-10 w-full flex flex-col h-full">
                   ...
                   {/* Testimonial */}
                   <p className="text-xl lg:text-2xl font-display font-medium text-white/90 leading-relaxed mb-8 drop-shadow-md pr-8 group-hover:text-white transition-colors duration-300">
                     {client.testimonial}
                   </p>
                   ...
                   <div className="flex items-center justify-between mt-auto pt-6 border-t border-white/10">
                     ...
                     <div className="px-3 py-1 bg-white/5 rounded-full text-[10px] font-bold tracking-[0.2em] uppercase text-white/60">
                       {client.year}
                     </div>
                   </div>
                </div>
        ...
        >
           {CLIENTS.map((client, index) => (
             <motion.div 
               ...
               className="relative h-[450px] w-[85vw] max-w-[340px] shrink-0 snap-center rounded-[2rem] overflow-hidden bg-[#121212] border border-white/10 flex flex-col p-6 shadow-[0_20px_40px_rgba(0,0,0,0.5)] group"
             >
                {/* Dynamic Gradient Background */}
                <div className={`absolute inset-0 bg-gradient-to-br ${client.gradient} opacity-50 z-0`} />
                <div className="absolute inset-0 opacity-[0.03] bg-[url('https://grainy-gradients.vercel.app/noise.svg')] z-0 pointer-events-none mix-blend-overlay" />
                
                <div className="absolute top-6 right-6 text-white/10 z-10">
                ...
                <div className="relative z-10 w-full flex flex-col h-full">
                   ...
                   <p className="text-lg font-display font-medium text-white/90 leading-relaxed mb-6 drop-shadow-md pr-4">
                     {client.testimonial}
                   </p>
                   
                   <div className="flex flex-col gap-3 mt-auto pt-5 border-t border-white/10">
                     <p className="text-white/60 text-[10px] font-bold tracking-widest uppercase">
                       {client.type}
                     </p>
                     <div className="w-max px-3 py-1 bg-white/5 rounded-full text-[10px] font-bold tracking-[0.2em] uppercase text-white/50">
                       {client.year}
                     </div>
                   </div>
                </div>
