    ...
    <section id="pricing" className="py-24 md:py-32 bg-[#0a0a0a] relative overflow-hidden">
      ...
      <div className="max-w-[1400px] mx-auto px-6 relative z-10 flex flex-col items-center">
        ...
        <div className="mb-12 flex flex-col items-center text-center">
           ...
           <h3 className="text-4xl md:text-5xl lg:text-7xl font-display font-bold text-white leading-tight tracking-tight mb-6">
             Select Your <span className="text-white/40">Architecture.</span>
           </h3>
           <p className="text-white/60 max-w-2xl text-lg md:text-xl">
             Choose a complete, done-for-you digital dominance suite, or use our builder to engineer a bespoke architecture.
           </p>
        </div>

        {/* Unified Toggle Tabs */}
        <div className="mb-20 bg-[#161a22] p-2 rounded-full border border-white/10 shadow-2xl flex items-center relative w-full max-w-[400px] mx-auto">
          {/* Active Tab Background Indicator */}
          <div 
            className="absolute top-2 bottom-2 w-[calc(50%-8px)] rounded-full bg-white transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)]"
            style={{ left: activeTab === 'prebuilt' ? '8px' : 'calc(50% + 0px)' }}
          />
          
          <button 
            onClick={() => setActiveTab('prebuilt')}
            className={`relative z-10 flex-1 py-3 text-sm font-bold uppercase tracking-widest transition-colors duration-300 ${activeTab === 'prebuilt' ? 'text-black' : 'text-white/60 hover:text-white'}`}
          >
            Pre-built
          </button>
          <button 
            onClick={() => setActiveTab('custom')}
            className={`relative z-10 flex-1 py-3 text-sm font-bold uppercase tracking-widest transition-colors duration-300 ${activeTab === 'custom' ? 'text-black' : 'text-white/60 hover:text-white'}`}
          >
            Custom
          </button>
        </div>
        ...
        <div className="w-full">
          {activeTab === 'prebuilt' ? (
                   ...
                   >
                     <div className="p-6 sm:p-8 lg:p-10 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 sm:gap-8">
                        ...
                        <div className="flex items-center gap-4 sm:gap-6 lg:gap-8 flex-1">
                           ...
                           <div>
                             <div className="flex items-center gap-3 sm:gap-4 mb-2 flex-wrap">
                               ...
                               {bundle.isPopular && (
                                 <span className="px-3 py-1 bg-primary text-white text-[10px] font-bold uppercase tracking-widest rounded-full shadow-[0_0_20px_rgba(0,102,204,0.5)] whitespace-nowrap">
                                   Most Popular
                                 </span>
                               )}
                             </div>
                        ...
                        <div className="flex items-center gap-6 sm:gap-8 lg:gap-12 lg:w-1/3 justify-between lg:justify-end w-full lg:w-auto mt-4 lg:mt-0 pt-4 lg:pt-0 border-t lg:border-t-0 border-current/10">
                           ...
                           <div className={`w-10 h-10 lg:w-12 lg:h-12 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors ${isSelected ? 'bg-black border-black text-white' : 'border-white/20 group-hover:border-white'}`}>
                              ...
                              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
                                {bundle.features.map((feature, i) => (
                                  <div key={i} className="flex items-center gap-3">
                                    <Check size={16} className={isSelected ? 'text-primary' : 'text-primary'} />
            ...
            <motion.div 
              ...
              exit={{ opacity: 0, y: -20 }}
              transition={{ duration: 0.5 }}
              className="flex flex-col-reverse lg:flex-row gap-8 lg:gap-12 items-start relative w-full"
            >
                         ...
                         {CUSTOM_ITEMS.filter(i => i.category === category).map(item => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <div className="w-8 h-8 rounded-full border border-white/20 flex items-center justify-center text-white/50 shrink-0">
                                  <Plus size={14} />
                                </div>
                              </motion.div>
                            )
               ...
               <div className="w-full lg:w-5/12 relative lg:sticky lg:top-24 z-20">
                  <div className="w-full min-h-[400px] lg:min-h-[600px] rounded-[2.5rem] bg-[#161a22] border border-white/10 shadow-[0_20px_40px_rgba(0,0,0,0.3)] p-6 md:p-8 flex flex-col relative overflow-hidden">
                    ...
                    <div className="relative z-10 flex-1 flex flex-col">
                      <div className="flex items-center justify-between border-b border-white/10 pb-6 mb-6">
                        <h4 className="text-lg md:text-xl font-bold text-white tracking-wide">Your Architecture</h4>
                      ...
                      <div className="flex-1 flex flex-col gap-3 min-h-[200px] lg:min-h-[300px]">
                         <AnimatePresence>
                           {selectedItems.length === 0 && (
                             ...
                             >
                                <div className="w-12 h-12 md:w-16 md:h-16 rounded-full border border-dashed border-white/20 flex items-center justify-center mb-4">
                                  <Plus size={20} className="text-white/20" />
                                </div>
                                <p className="text-white/40 font-medium text-sm md:text-base">Select modules to begin constructing your architecture.</p>
                             </motion.div>
                           )}
                         </AnimatePresence>
                         ...
                         <div className="flex justify-between items-end">
                           ...
                           <div className="text-right">
                             ...
                             <span className="text-xl md:text-2xl font-display font-bold text-white">
                               <AnimatedNumber value={totalMonthly} /><span className="text-xs md:text-sm text-white/40">/mo</span>
                             </span>
                           </div>
                         </div>
                         <button 
                           onClick={handleCustomDeploy}
                           className="w-full py-4 rounded-xl font-bold bg-primary text-white hover:scale-[1.02] transition-transform shadow-[0_0_20px_rgba(0,102,204,0.4)] flex items-center justify-center gap-2 text-sm md:text-base"
                         >
                           Deploy via WhatsApp <ArrowRight size={16} />
                         </button>
                      </div>
